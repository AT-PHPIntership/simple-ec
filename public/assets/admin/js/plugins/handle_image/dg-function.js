var dgUI = {
	...
	category:{
		...
		add: function(id){
			UIModals.init(base_url+'/admin/clipart/edit-category');
		},
		remove: function(id){
			var remove = confirm(dgUI.category.lang.confirm_delete);
			if (remove == true)
			{
				jQuery.get( base_url+'/admin/clipart/delete-category/'+ id, function() {
		...
		edit: function(id){
			UIModals.init(base_url+'/admin/clipart/edit-category/'+id);
		},
		ini: function(){
		...
		tree: function(e, type){
			jQuery(e).dynatree({				
				...
				initAjax: {
					url: base_url+"/admin/clipart/categories/"
				},
				...
				onClick: function (node, event) {
					...
					if (event.target.className == 'dynatree-title')
					{
						...
						jQuery.ajax({url: base_url + '/admin/clipart/arts/1/' + cateId+'/1'}).done(function( data ) {
	...
	art:{
		...
		saveCategory: function(id)
		{
			...
			jQuery.ajax({			
				type: 'post',
				url: base_url+'/admin/clipart/save-category',
	...
	product:{
		...
		addCategoryJs: function(e, action){
			...
			if(action == 'save')
			{
				...
				var url = base_url + '/admin/product/category/add';						
		...
		setup: function(position, number){
			...
			jQuery.each(items, function(i, item){
				...
				if(item.id == 'area-design'){
					...
					html = html + '<img width="50" height="50" alt="" src="'+ item.img+ '" />';
		...
		removeCate: function(e){
			...
			var url = base_url + '/admin/product/category/remove';						
		...
		fonts:{
			...
			save: function(e){
				...
				jQuery.ajax({
					...
					type: 'POST',
					url: 'http://'+location.host + "/admin/font/add-google-font",					
					...
					success: function(data){
						...
						location.replace('http://'+location.host + "/admin/font/");
					}
				});
			}
		},
